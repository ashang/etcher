...
analytics.service('AnalyticsService', function($log, $window, $mixpanel, SettingsModel) {
  ...
  this.logEvent = (message, data) => {
    ...
    const anonymizedFlatStartCaseData = utils.hideObjectAbsolutePaths(flatStartCaseData);

    if (SettingsModel.get('errorReporting') && isRunningInAsar()) {
      $mixpanel.track(message, anonymizedFlatStartCaseData);
    }

    const debugMessage = _.attempt(() => {
      if (anonymizedFlatStartCaseData) {
        return `${message} (${JSON.stringify(anonymizedFlatStartCaseData)})`;
      }
